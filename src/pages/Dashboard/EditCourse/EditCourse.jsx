...
const EditCourse = () => {
  ...
  // Dynamic field states (same pattern as AddCourse)
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      try {
        setLoading(true);
        const res = await axiosSecure.get(`/api/admin/courses/${id}`);
        ...
        // ---- setup learning outcome dynamic ids ----
        if (course.learningOutcomes && course.learningOutcomes.length) {
          setLearningOutcomeIds(
            course.learningOutcomes.map((_, idx) => idx)
          );
        } else {
          setLearningOutcomeIds([0]);
        }

        // ---- setup modules dynamic ids ----
        if (course.modules && course.modules.length) {
          const mappedModules = course.modules.map((mod, mIndex) => ({
            id: mIndex,
            lessonIds:
              mod.lessons && mod.lessons.length
                ? mod.lessons.map((_, lIndex) => lIndex)
                : [0],
            quizQuestionIds:
              mod.quiz && mod.quiz.length
                ? mod.quiz.map((_, qIndex) => qIndex)
                : [0],
          }));
          setModules(mappedModules);
        } else {
          setModules([{ id: 0, lessonIds: [0], quizQuestionIds: [0] }]);
        }
      } catch (err) {
        console.error(err);
        Swal.fire({
  ...
  // ============ Dynamic handlers (same as AddCourse) ============
  ...
  const submitToBackend = async (data, status) => {
    const coursePayload = {
      ...data,
      status: status || initialCourse?.status || "draft",
    };

    if (coursePayload.isFree) {
      coursePayload.price = 0;
      coursePayload.discountPrice = 0;
    }
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-4">
        <div>
          ...
          <p className="text-sm text-base-content/70">
            Update course information, curriculum, quizzes and assignments.
          </p>
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="card bg-base-100 shadow-md">
            <div className="card-body space-y-3">
              ...
              {learningOutcomeIds.map((oid, index) => (
                <div key={oid} className="flex gap-2">
                  ...
                  {learningOutcomeIds.length > 1 && (
                    <button
                      ...
                      onClick={() => removeOutcomeField(oid)}
                    >
                      ✕
                    </button>
                  )}
